...
이미 앱이 실행 중이라면 기존 프로세스를 완전히 종료한 후 새로 패키징한 앱을 실행해야 변경 사항이 적용됩니다. 대화는 사용자 설정 폴더의 Markdown 파일로 저장되어 앱을 종료해도 유지됩니다. 서버 URL, 선택 모델과 API 키는 아직 현재 실행 중에만 사용되며 앱을 종료하면 다시 입력해야 합니다.
...
앱을 시작하면 저장된 대화 목록을 표시합니다. 목록에서 이전 대화를 열거나 **새 대화**를 선택해 대화를 시작할 수 있습니다. 저장된 대화가 없으면 빈 새 대화가 자동으로 열립니다.

대화는 사용자 설정 폴더 아래의 `Agent Chat/conversations`에 대화별 Markdown 파일로 저장됩니다. 각 파일에는 ID·생성·수정 시각의 frontmatter와 사람이 읽을 수 있는 메시지 본문이 들어 있습니다. 대화 삭제와 마지막으로 열었던 대화의 자동 복원은 현재 테스트 단계에서 제공하지 않습니다.

API 키는 Markdown 파일에 저장하지 않습니다. 연결 프로필과 API 키의 영속 저장은 이후 기능으로 계획되어 있습니다.
...
첫 번째 목표인 로컬 LLM 채팅 UI와 Markdown 기반 대화 저장을 완료했습니다. Wails 데스크톱 앱에서 실제 OpenAI 호환 서버의 모델을 조회하고 선택한 모델과 스트리밍 방식으로 대화하는 흐름을 확인했습니다. 생성 중단도 구현되어 있으며, 다음 단계는 연결 프로필 저장과 API 키의 안전한 보관입니다.
