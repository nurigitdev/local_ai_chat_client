...
- 일회용 코드 승인 후 수동 동기화하는 같은 네트워크의 벤치마크 결과 동기화
...
왼쪽의 **결과 동기화** 작업 공간에서 같은 네트워크의 다른 PC와 벤치마크 결과를 수동으로 주고받을 수 있습니다. 한쪽 PC가 표시한 주소와 일회용 코드를 다른 쪽 PC에 입력하고, 원래 PC에서 연결 요청을 승인하면 연결됩니다. 연결 후 **동기화**를 누르면 상대 결과를 먼저 받고, 이어서 이 PC의 결과도 보냅니다. 자동 탐색과 백그라운드 자동 동기화는 제공하지 않습니다.
...
동기화 기록은 이 PC에만 저장하고 전송하지 않습니다. 각 항목에는 시각, 상대 PC 이름, 방향(동기화·전송·수신), 완료·실패 상태, 추가·중복·제외·충돌 건수와 오류를 저장합니다. 최근 200건을 보관하며 사용자가 기록을 비울 수 있습니다.
