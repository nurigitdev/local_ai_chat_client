...
첫 번째 목표인 로컬 LLM 채팅 UI와 Markdown 기반 대화 저장을 완료했습니다. 마지막 LLM 서버 URL도 복원하며, 모델은 서버에서 다시 조회하고 API 키는 저장하지 않습니다. OpenAI 호환 서버가 제공한 토큰 사용량과 앱이 측정한 응답 성능도 화면과 대화 기록에서 확인할 수 있습니다. 다음 단계는 여러 연결 프로필 관리와 필요할 경우 API 키의 운영체제 보안 저장소 연동입니다.
