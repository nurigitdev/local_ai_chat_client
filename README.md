...
**기록 비교**에서는 먼저 **문서·한국어 실무** 또는 **개발·지시 이행** 질문지를 고른 뒤 A·B·C 기록을 선택합니다. 선택한 질문지의 기록만 목록에 표시하며, 각 항목은 `모델 · 추론 강도 · 연결 프로필 · 실행 시각` 순서로 표시합니다. 추론 강도 정보가 없는 기존 기록은 **자동(서버 기본값)**으로 표시합니다.

모델 실험실 왼쪽 사이드바의 **내 벤치마크**와 **가져온 벤치마크**는 제목 행을 눌러 각각 접거나 펼칠 수 있습니다. 열린 상태는 앱을 다시 실행해도 유지하며, 제목 오른쪽 숫자는 현재 사이드바에 표시된 기록 수입니다.
