...
- **제안:** 대화, 메시지 및 연결 프로필 메타데이터는 SQLite에 저장합니다.
- **제안:** API 키는 운영체제 보안 저장소를 사용하고 데이터에는 참조 값만 저장합니다.
...
- 요청 취소 및 경쟁 상태 테스트
- 저장소 마이그레이션 테스트
- `httptest.Server`를 사용한 OpenAI 호환 API 통합 테스트
...
| 프런트엔드 프레임워크 | 제안 | React + TypeScript |
| 로컬 데이터베이스 | 제안 | SQLite |
