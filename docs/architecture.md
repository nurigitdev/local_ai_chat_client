...
- 완료된 벤치마크 결과 창과 기록 분석·비교 화면에서 Markdown·HTML 보고서 내보내기 제공. 기본 파일명에는 모델·질문지를 사용하고, 비교는 A 모델과 나머지 기록 수를 사용해 짧게 구성
- 오류, 재연결 및 작업 상태 표시
...
- 모델 벤치마크의 테스트 항목·응답·성능 수치를 별도 Markdown 파일로 저장하고, Markdown·HTML 보고서 안에 복원용 원본 데이터를 포함해 다른 환경의 결과를 가져올 수 있도록 함. 보고서 기본 파일명은 단일 기록일 때 `benchmark_[모델]__[질문지]`, 두·세 기록 비교일 때 `benchmark_[A 모델]_vs_1other|2others__[질문지]`이며, 파일 시스템에서 허용하지 않는 문자는 공백으로 정리함. 가져올 때는 저장된 결과와 ID·가져온 상태를 제외한 내용의 지문을 비교해 같은 결과를 제외함
...
같은 사이드바에 선택한 모델의 채팅 누적 입력·출력·전체 토큰을 표시합니다. 서버가 제공한 사용량을 합산하며, 모델 변경·모델 목록 새로고침·서버 주소 변경·앱 재시작 시 초기화됩니다. 누적 사용량은 컨텍스트 잔여량을 나타내지 않습니다. 사이드바는 오른쪽 위 `‹` 버튼으로 숨기고, 접힌 상태에서 화면 왼쪽 위 `›` 버튼으로 다시 열 수 있으며 표시 상태는 저장하지 않습니다.
