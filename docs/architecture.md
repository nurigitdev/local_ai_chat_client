...
- 단일 사용자용 데스크톱 애플리케이션
- OpenAI 호환 모델 목록 조회
- 텍스트·코드·텍스트 기반 PDF 첨부를 포함한 스트리밍 채팅
- 생성 중단과 오류 처리
- Markdown 파일 기반 대화 기록의 로컬 저장

### 후속 범위

- Office 문서·이미지를 포함한 멀티모달 메시지
- 도구 호출과 구조화된 응답
- 여러 AI Provider 어댑터
...
- 대화 및 메시지 목록 표시
- 연결 프로필과 모델 선택
- 사용자 입력, 텍스트·코드·텍스트 기반 PDF 첨부와 생성 중단 요청
- 스트리밍 델타를 수신해 응답 갱신
...
- 대화·메시지와 첨부한 텍스트·코드·텍스트 기반 PDF의 추출 내용을 사용자 설정 폴더의 Markdown 파일로 저장
- 대화 목록용 제목과 요약 메타데이터를 Markdown 파일에서 조회
- 모델 벤치마크의 테스트 항목·응답·성능 수치를 별도 Markdown 파일로 저장
...
- 모델 벤치마크는 저장된 연결 프로필 하나와 모델 하나를 사용합니다. 지시 이행·추론·코드·한국어 설명의 기본 프롬프트는 실행 전에 사용자가 수정할 수 있으며, 테스트는 서버 부하가 서로 영향을 주지 않도록 순차로 생성합니다. 테스트 질문·응답·응답 시간·첫 토큰 시간·토큰 사용량은 `model-benchmarks/{id}.md`에 저장합니다. 결과는 현재 실행 환경의 측정 기록이며 범용 모델 순위를 의미하지 않습니다.
