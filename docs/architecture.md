...
- 모델 벤치마크의 테스트 항목·응답·성능 수치를 별도 Markdown 파일로 저장하고, Markdown·HTML 보고서 안에 복원용 원본 데이터를 포함해 다른 환경의 결과를 가져올 수 있도록 함. 가져올 때는 저장된 결과와 ID·가져온 상태를 제외한 내용의 지문을 비교해 같은 결과를 제외함
